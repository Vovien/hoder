...
public class BillDetailDiecountAdapter extends RecyclerView.Adapter<BillDetailDiecountAdapter.MyBillDetailViewHolder> {
    private Context context;
    private LayoutInflater inflater;
    private List<SalesOrderE> salesOrderEList;
    private List<Object> datas;

    public BillDetailDiecountAdapter(Context context) {
     ...
     * 折扣明细
     */
    private void setDiscountDetailsData() {
        datas = new ArrayList<>();
        length = 0;
        for (int i = 0; i < salesOrderEList.size(); i++) {
            if (salesOrderEList.get(i) != null && salesOrderEList.get(i).getArrayOfSalesOrderDiscountE() != null && salesOrderEList.get(i).getArrayOfSalesOrderDiscountE().size() != 0) {
                datas.addAll(salesOrderEList.get(i).getArrayOfSalesOrderDiscountE());
                length++;
            }
        }
    }
    ...
    public MyBillDetailViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = inflater.inflate(R.layout.item_bill_detail_discount, parent, false);
    ...
    public void onBindViewHolder(MyBillDetailViewHolder holder, int position) {
        ...
        /***折扣明细***/
        if (datas.get(position) instanceof SalesOrderDiscountE) {
            holder.item_3_Name.setText(((SalesOrderDiscountE) datas.get(position)).getDiscountItem());
            holder.item_3_Number.setText(context.getString(R.string.minus_amount_str, ArithUtil.stripTrailingZeros(((SalesOrderDiscountE) datas.get(position)).getDiscountAmount())));
        }
