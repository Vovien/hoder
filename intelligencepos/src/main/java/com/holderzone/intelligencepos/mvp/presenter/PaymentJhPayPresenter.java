...
public class PaymentJhPayPresenter extends BasePresenter<PaymentJhPayContract.View> implements PaymentJhPayContract.Presenter {
    ...
    public PaymentJhPayPresenter(PaymentJhPayContract.View view) {
        super(view);
    }

    @Override
    public void submitJhPay(String saleOrderGUID, String paymentItemCode, double payableAmount, String transactionNumber) {
        mRepository.getUsers()
                .flatMap(users -> {
                    ...
                    return XmlData.Builder()
                            .setRequestMethod(RequestMethod.SalesOrderPaymentB.AddJhPay)
                ...
                .subscribe(new Observer<XmlData>() {
                    ...
                    public void onNext(XmlData xmlData) {
                        ...
                        if (ApiNoteHelper.checkBusiness(xmlData)) {
                            ...
                            if (mainSalesOrderE != null) {
                                mView.onJhPaySucceed(mainSalesOrderE);
                            } else {
                                mView.onJhPayFailed("数据非法，SalesOrderE不得为null。");
                            }
                        } else {
                            if (ApiNoteHelper.checkAuthorizationWhenBusinessFailed(xmlData)) {
                                ...
                                mView.onJhPayFailed(ApiNoteHelper.obtainErrorMsg(xmlData));
                            }
                        }
                        // 停止轮询
                    ...
                    public void onError(Throwable e) {
                        e.printStackTrace();
                        mView.hideIntervalDialog();
                        mView.onJhPayFailed(e.getMessage());
                    }
