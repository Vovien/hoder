...
import com.holderzone.intelligencepos.mvp.presenter.RemindDishesPresenter;
 ...
 */
public class RemindDishesActivity extends BaseActivity<RemindDishesContract.Presenter> implements RemindDishesContract.View {
    ...
    public static Intent newIntent(Context context, DiningTableE diningTableE) {
        Intent intent = new Intent(context, RemindDishesActivity.class);
    ...
    protected int getContentViewLayoutId() {
        return R.layout.activity_remind_dishes;
    }

    @Override
    protected RemindDishesContract.Presenter initPresenter() {
        return new RemindDishesPresenter(this);
    }
    ...
    @Override
    public void onRequestCalledUpDishesSucceed(List<SalesOrderBatchDishesE> arrayOfSalesOrderBatchDishesE) {
    ...
    @Override
    public void onRequestCalledUpDishesFailed() {
        // do nothing
    }

    @Override
    public void onRemindSucceed() {
        // 返回桌台界面
        AppManager.getInstance().finishUntil(TableActivity.class);
    }

    @Override
    public void onRemindFailed() {
        // do nothing
    }
    ...
    private void initTitle() {
        mTitle.setTitleText(mDiningTableE.getName() + " 催菜");
        mTitle.setOnReturnClickListener(this::finish);
        mTitle.setOnMenuClickListener(this::requestCalledUpDishes);
    }
    ...
    private void initAdapter() {
        mSalesOrderBatchDishesAdapter = new CommonAdapter<SalesOrderBatchDishesE>(RemindDishesActivity.this, R.layout.item_remind, mSalesOrderBatchDishesEs) {
            @Override
...
//                    holder.setVisible(R.id.tv_practice_price, true);
    ...
    private void initButton() {
        ...
        RxView.clicks(mBtnConfirm).throttleFirst(1, TimeUnit.SECONDS).subscribe(o -> {
            ...
            mPresenter.submitRemindDishes(temp);
        });
    }
     ...
     * 请求菜品数据
     */
    private void requestCalledUpDishes() {
        // 刷新中 置位
        setRefreshing(true);
        // 请求新的网络数据
        mPresenter.requestCalledUpDishes(mDiningTableE.getSalesOrderE().getSalesOrderGUID(), 1);
    }
...
//            mTvSelectAll.setText("取消全选");
        ...
        if (mArrayOfSalesOrderBatchDishesGUIDSelected.size() == 0) {
            ...
            mBtnConfirm.setText(mArrayOfSalesOrderBatchDishesGUIDSelected.size() + "菜品 确认催菜");
        }
    }
