...
public class PaymentWxPayPresenter extends BasePresenter<PaymentWxPayContract.View> implements PaymentWxPayContract.Presenter {
    ...
    public PaymentWxPayPresenter(PaymentWxPayContract.View view) {
        super(view);
    }

    @Override
    public void submitWxPay(String saleOrderGUID, String paymentItemCode, double payableAmount, String transactionNumber) {
        mRepository.getUsers()
                .flatMap(users -> {
                    ...
                    return XmlData.Builder()
                            .setRequestMethod(RequestMethod.SalesOrderPaymentB.AddWeChat)
                ...
                .subscribe(new Observer<XmlData>() {
                    ...
                    public void onNext(XmlData xmlData) {
                        ...
                        if (ApiNoteHelper.checkBusiness(xmlData)) {
                            ...
                            if (mainSalesOrderE != null) {
                                mView.onWxPaySucceed(mainSalesOrderE);
                            } else {
                                mView.onWxPayFailed("数据非法，SalesOrderE不得为null。");
                            }
                        } else {
                            if (ApiNoteHelper.checkAuthorizationWhenBusinessFailed(xmlData)) {
                                ...
                                mView.onWxPayFailed(ApiNoteHelper.obtainErrorMsg(xmlData));
                            }
                        }
                        // 停止轮询
                    ...
                    public void onError(Throwable e) {
                        e.printStackTrace();
                        mView.hideIntervalDialog();
                        mView.onWxPayFailed(e.getMessage());
                    }
